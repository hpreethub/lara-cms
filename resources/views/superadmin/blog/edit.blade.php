...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            @if(isset($data))
            <form class="form-horizontal" method="POST" action="{{route('superadmin.blog.update',$data->id)}}" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <fieldset>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <textarea class="form-control" id="textarea2" name="meta_description" required>{{$data->meta_description}}</textarea>
                        </div>
                    </div>
